...
var MNEMO_COMPACT_CAPTURE = process.env.MNEMO_COMPACT_CAPTURE !== "false";
var MNEMO_COMPACT_MIN_IMPORTANCE = parseFloat(process.env.MNEMO_COMPACT_MIN_IMPORTANCE || "0.5");
var MNEMO_COMPACT_MAX_MEMORIES = parseInt(process.env.MNEMO_COMPACT_MAX_MEMORIES || "10", 10);
...
IMPORTANT: Output ONLY the JSON array, no other text. If nothing worth storing, return empty array [].`;
  try {
    const resp = await fetch(`${OLLAMA_BASE_URL}/api/chat`, {
      ...
      body: JSON.stringify({
        ...
        options: {
          temperature: 0.3,
          num_predict: 8192
          // Larger budget for deep extraction
        }
      })
    });
    if (!resp.ok) {
    ...
    log2(`LLM extraction failed: ${err}`);
    return [];
  }
}
...
async function readTranscript(transcriptPath) {
  try {
    ...
    log2(`Failed to read transcript: ${err}`);
    return "";
  }
}
async function main() {
  if (!MNEMO_COMPACT_CAPTURE) {
    log2("Compact-capture disabled by environment");
    process.exit(0);
  }
  let input = {};
  try {
  ...
  if (!transcriptPath) {
    log2("No transcript path provided, skipping");
    process.exit(0);
  }
  log2(`Reading transcript: ${transcriptPath}`);
  const transcriptText = await readTranscript(transcriptPath);
  if (transcriptText.length < 100) {
    log2("Transcript too short, skipping");
    process.exit(0);
  }
  log2(`Transcript length: ${transcriptText.length} chars`);
  ...
  log2(`Done: stored ${stored} memories from compact`);
  process.exit(0);
}
main().catch((err) => {
