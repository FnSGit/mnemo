...
var MNEMO_AUTO_RECALL = process.env.MNEMO_AUTO_RECALL !== "false";
var MNEMO_RECALL_LIMIT = parseInt(process.env.MNEMO_RECALL_LIMIT || "5", 10);
var MNEMO_MIN_SCORE = parseFloat(process.env.MNEMO_MIN_SCORE || "0.3");
...
async function searchMemories(query, limit2) {
  try {
    const mnemo = await getMnemo();
    const results = await mnemo.recall(query, { limit: limit2 || 5 });
    return results.map((r) => ({
      id: r.id,
...
async function detectProjectContext(cwd) {
  ...
  try {
    ...
    try {
      await fs.access(path2.join(cwd, "pyproject.toml"));
      context.language = "python";
    } catch {
    }
    try {
      await fs.access(path2.join(cwd, "go.mod"));
      context.language = "go";
    } catch {
    }
  } catch (err) {
    log2(`Project detection error: ${err}`);
  }
  return context;
}
...
async function main() {
  if (!MNEMO_AUTO_RECALL) {
    log2("Auto-recall disabled by environment");
    process.exit(0);
  }
  let input = {};
  try {
  ...
  log2(`Session start in: ${cwd} (scope: ${MNEMO_SCOPE})`);
  const projectContext = await detectProjectContext(cwd);
  log2(`Detected: ${projectContext.language}/${projectContext.framework}`);
  const queries = [];
  ...
  for (const query of queries) {
    ...
    for (const mem of results) {
      if (!seenIds.has(mem.id) && mem.score >= MNEMO_MIN_SCORE) {
        ...
        if (memScope === "global" || memScope === MNEMO_SCOPE) {
          seenIds.add(mem.id);
  ...
  log2(`Found ${allMemories.length} relevant memories`);
